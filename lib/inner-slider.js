    ...
    function InnerSlider(props) {
      ...
      _defineProperty(
        ...
        function() {
          ...
          if (!_this.props.infinite && _this.props.variableWidth) {
            var widthUntilListRightEnd = 0;

            var trackElem = _reactDom["default"].findDOMNode(_this.track);

            var listWidth = Math.ceil(
              (0, _innerSliderUtils.getWidth)(
                _reactDom["default"].findDOMNode(_this.list)
              )
            );

            for (
              var slide = _this.state.currentSlide;
          ...
          _this.setState({
            slidesToShow: realSlideShowed || _this.props.slidesToShow
          });
        }
      );
